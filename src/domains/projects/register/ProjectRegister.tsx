  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (projectID) {
        ...
        setRedirecting(true);
        router.push(`/projects/${projectID}`); // ✅ updated route
      } else {
        toast.error("Project registered but no ID returned. Please try again.");
      }
    } catch (error: any) {
      console.error("Submission error:", error);
      const errMsg =
        error.response?.data?.error?.message || "Project Registration Failed, Try Again!";
      setError(errMsg);
      toast.error(errMsg, {
    ...
    <>
      ...
      {redirecting && (
        <div className="flex items-center justify-center h-screen">
          <p className="text-lg font-medium">Redirecting to project page...</p>
        </div>
      )}
    </>
  );
